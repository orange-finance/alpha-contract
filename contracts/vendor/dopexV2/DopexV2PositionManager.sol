...
contract DopexV2PositionManager is Ownable, ReentrancyGuard, Multicall {
    ...
    modifier onlyWhitelistedHandlers(IHandler _handler) {
        if (!whitelistedHandlers[address(_handler)]) revert DopexV2PositionManager__NotWhitelistedHandler();
        _;
    }
    ...
    function mintPosition(
        IHandler _handler,
        bytes calldata _mintPositionData
    ) external onlyWhitelistedHandlers(_handler) nonReentrant returns (uint256 sharesMinted) {
    ...
    function burnPosition(
        IHandler _handler,
        bytes calldata _burnPositionData
    ) external onlyWhitelistedHandlers(_handler) nonReentrant returns (uint256 sharesBurned) {
