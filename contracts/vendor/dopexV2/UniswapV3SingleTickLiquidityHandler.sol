 ...
 */
contract UniswapV3SingleTickLiquidityHandler is ERC1155(""), IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    modifier onlyWhitelisted() {
        if (!whitelistedApps[msg.sender]) revert UniswapV3SingleTickLiquidityHandler__NotWhitelisted();
        _;
    }
    ...
    function mintPositionHandler(
        ...
        uint256 tokenId = uint256(
            keccak256(abi.encode(address(this), _params.pool, _params.tickLower, _params.tickUpper))
        );
        ...
        if (amount0 > 0 && amount1 > 0) revert UniswapV3SingleTickLiquidityHandler__InRangeLP();
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 0 || tki.tokensOwed1 > 0) {
                ...
                (uint128 liquidityFee, , , ) = UniswapV3SingleTickLiquidityHandler(address(this)).addLiquidity(
                    LiquidityManager.AddLiquidityParams({
                        token0: tki.token0,
                        token1: tki.token1,
                        fee: tki.fee,
    ...
    function burnPositionHandler(
        ...
        (uint256 amount0, uint256 amount1) = _params.pool.burn(_params.tickLower, _params.tickUpper, liquidityToBurn);
...

        {
            ...
            feesOwedToken0 = uint128((tki.tokensOwed0 * userLiquidity0) / totalLiquidity0);
            feesOwedToken1 = uint128((tki.tokensOwed1 * userLiquidity1) / totalLiquidity1);
        }
    ...
    ) external onlyWhitelisted whenNotPaused returns (address[] memory, uint256[] memory, uint256) {
        UsePositionParams memory _params = abi.decode(_usePositionHandler, (UsePositionParams));
        uint256 tokenId = uint256(
            keccak256(abi.encode(address(this), _params.pool, _params.tickLower, _params.tickUpper))
        );
    ...
    function unusePositionHandler(
        bytes calldata _unusePositionData
    ) external onlyWhitelisted whenNotPaused returns (uint256[] memory, uint256) {
        UnusePositionParams memory _params = abi.decode(_unusePositionData, (UnusePositionParams));
     ...
     */
    function getHandlerIdentifier(bytes calldata _data) external view returns (uint256 handlerIdentifierId) {
        (IUniswapV3Pool pool, int24 tickLower, int24 tickUpper) = abi.decode(_data, (IUniswapV3Pool, int24, int24));

        return uint256(keccak256(abi.encode(address(this), pool, tickLower, tickUpper)));
    }
    ...
    function tokensToPullForDonate(
        ...
        DonateParams memory _params = abi.decode(_donatePosition, (DonateParams));
     ...
     */
    function updateLockedBlockDuration(uint64 _newLockedBlockDuration) external onlyRole(DEFAULT_ADMIN_ROLE) {
