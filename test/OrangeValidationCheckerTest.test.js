...
describe("OrangeValidationCheckerTest", function () {
  before(async () => {
    //import
    [alice, bob, carol] = await ethers.getSigners();
    const OrangeParametersV1 = await ethers.getContractFactory(
      "OrangeParametersV1"
    );
    params = await OrangeParametersV1.deploy();
    const OrangeValidationCheckerMock = await ethers.getContractFactory(
      "OrangeValidationCheckerMock"
    );
    mock = await OrangeValidationCheckerMock.deploy(params.address);
  ...
  describe("exec", function () {
    ...
    it("fail", async () => {
      await expect(mock.connect(alice).exec(proof1)).to.revertedWith("201");
    });
    it("fail2", async () => {
      await expect(mock.connect(carol).exec(proof0)).to.revertedWith("201");
    });
  });
});
